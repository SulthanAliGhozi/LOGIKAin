...
import { canStaff, type StaffPermission } from '../../lib/auth/permissions'
...
async function staffClient(permission: StaffPermission) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Unauthenticated')
  const { data: profile } = await supabase.from('profiles').select('role,status').eq('id', user.id).maybeSingle()
  if (!profile || profile.status !== 'active' || !canStaff(profile.role, permission)) throw new Error('Forbidden')
  return { supabase, user }
}

async function privilegedClient(permission: StaffPermission) {
  return staffClient(permission)
}

export async function createAdminLead(input: unknown) {
  const data = leadSchema.parse(input); const { supabase, user } = await staffClient('leads')
...
export async function addClientNote(input: unknown) {
  ...
  const { supabase, user } = await staffClient('clients')
...
export async function createAdminProject(input: unknown) {
  const data = projectSchema.parse(input); const { supabase, user } = await staffClient('delivery')
...
export async function saveService(input: unknown) {
  const data = serviceSchema.parse(input); const { supabase, user } = await staffClient('content'); const { id, ...fields } = data
...
export async function updateMediaAsset(input: unknown) {
  ...
  const { supabase, user } = await staffClient('media'); const { error } = await supabase.from('media_assets').update({ alt_text: data.alt_text, is_decorative: data.is_decorative }).eq('id', data.id)
  if (error) throw new Error(error.message)
...
export async function updateInvoiceStatus(id: string, status: string) {
  const validStatus = z.enum(['draft','issued','partially_paid','paid','overdue','void']).parse(status); const { supabase, user } = await staffClient('finance')
...
export async function recordPayment(input: unknown) {
  const data = z.object({ invoice_id: z.string().uuid(), amount_minor: z.number().int().positive(), currency: z.string().length(3).default('IDR'), provider: z.string().optional(), provider_reference: z.string().optional() }).parse(input); const { supabase, user } = await staffClient('finance')
...
export async function updateTicketStatus(id: string, status: string) {
  const validStatus = z.enum(['open','in_progress','waiting_client','resolved','closed']).parse(status); const { supabase, user } = await staffClient('support')
...
export async function createMilestone(input: unknown) {
  const data = z.object({ project_id: z.string().uuid(), title: z.string().min(2), description: z.string().default(''), target_date: z.string().optional(), client_visible: z.boolean().default(true) }).parse(input); const { supabase, user } = await staffClient('delivery')
...
export async function createProjectTask(input: unknown) {
  const data = z.object({ project_id: z.string().uuid(), milestone_id: z.string().uuid().optional(), title: z.string().min(2), description: z.string().default(''), due_date: z.string().optional(), client_visible: z.boolean().default(false) }).parse(input); const { supabase, user } = await staffClient('delivery')
...
export async function requestProjectApproval(input: unknown) {
  const data = z.object({ project_id: z.string().uuid(), milestone_id: z.string().uuid().optional(), title: z.string().min(2), request_note: z.string().min(2) }).parse(input); const { supabase, user } = await staffClient('delivery')
...
export async function convertLeadToClient(leadId: string) {
  const { supabase, user } = await staffClient('clients'); const { data: lead, error: leadError } = await supabase.from('leads').select('*').eq('id', leadId).single(); if (leadError || !lead) throw new Error('Lead not found')
...
export async function snapshotInvoice(invoiceId: string) {
  const { supabase, user } = await staffClient('finance'); const { data: invoice, error: readError } = await supabase.from('invoices').select('*').eq('id', invoiceId).single(); if (readError || !invoice) throw new Error('Invoice not found')
...
export async function uploadProjectFile(formData: FormData) {
  ...
  const { supabase, user } = await staffClient('delivery')
...
export async function updateProjectFileVisibility(fileId: string, clientVisible: boolean) {
  const id = z.string().uuid().parse(fileId); const { supabase, user } = await staffClient('delivery')
...
export async function createTestimonial(input: unknown) {
  ...
  const { supabase, user } = await staffClient('content'); const { data: item, error } = await supabase.from('testimonials').insert({ ...data, created_by: user.id }).select('id').single()
...
export async function addInvoiceItem(input: unknown) {
  const data = z.object({ invoice_id: z.string().uuid(), description: z.string().min(2), quantity: z.number().int().positive(), unit_amount_minor: z.number().int().nonnegative(), sort_order: z.number().int().nonnegative().default(0) }).parse(input); const { supabase, user } = await staffClient('finance')
...
export async function restoreContentRevision(input: unknown) {
  const data = z.object({ table: contentTypeSchema, content_id: z.string().uuid(), revision_id: z.string().uuid() }).parse(input); const { supabase, user } = await privilegedClient('content')
...
export async function createRedirect(input: unknown) {
  const data = z.object({ source_path: z.string().startsWith('/'), target_path: z.string().startsWith('/'), status_code: z.enum(['301','302']).transform(Number), reason: z.string().optional() }).parse(input); const { supabase, user } = await staffClient('seo')
...
export async function deleteTestimonial(id: string) {
  const itemId = z.string().uuid().parse(id); const { supabase, user } = await staffClient('content'); const { error } = await supabase.from('testimonials').delete().eq('id', itemId); if (error) throw new Error(error.message); await supabase.from('activity_logs').insert({ actor_id: user.id, entity_type: 'testimonial', entity_id: itemId, action: 'deleted' }); revalidatePath('/'); revalidatePath('/admin/testimonials')
}

export async function deleteMediaAsset(id: string) {
  const itemId = z.string().uuid().parse(id); const { supabase, user } = await staffClient('media'); const { data: asset } = await supabase.from('media_assets').select('storage_path').eq('id', itemId).single(); if (!asset) throw new Error('Media asset not found'); const { error } = await supabase.from('media_assets').delete().eq('id', itemId); if (error) throw new Error(error.message); await supabase.storage.from('public-media').remove([asset.storage_path]); await supabase.from('activity_logs').insert({ actor_id: user.id, entity_type: 'media_asset', entity_id: itemId, action: 'deleted' }); revalidatePath('/admin/media')
}

export async function deleteRedirect(id: string) {
  const itemId = z.string().uuid().parse(id); const { supabase, user } = await staffClient('seo'); const { error } = await supabase.from('redirects').delete().eq('id', itemId); if (error) throw new Error(error.message); await supabase.from('activity_logs').insert({ actor_id: user.id, entity_type: 'redirect', entity_id: itemId, action: 'deleted' }); revalidatePath('/admin/seo')
}

export async function createClientRecord(input: unknown) {
  const data = z.object({ name: z.string().min(2), legal_name: z.string().optional(), email: z.string().email().optional().or(z.literal('')), phone: z.string().optional(), status: z.enum(['prospect','active','inactive','archived']).default('prospect') }).parse(input); const { supabase, user } = await staffClient('clients'); const { data: client, error } = await supabase.from('clients').insert({ ...data, email: data.email || null }).select('id').single(); if (error) throw new Error(error.message); await supabase.from('activity_logs').insert({ actor_id: user.id, entity_type: 'client', entity_id: client.id, action: 'created' }); revalidatePath('/admin/clients'); return client
}

export async function updateClientRecord(input: unknown) {
  const data = z.object({ id: z.string().uuid(), name: z.string().min(2), legal_name: z.string().optional(), email: z.string().email().optional().or(z.literal('')), phone: z.string().optional(), status: z.enum(['prospect','active','inactive','archived']) }).parse(input); const { supabase, user } = await staffClient('clients'); const { id, ...fields } = data; const { error } = await supabase.from('clients').update({ ...fields, email: fields.email || null, updated_at: new Date().toISOString() }).eq('id', id); if (error) throw new Error(error.message); await supabase.from('activity_logs').insert({ actor_id: user.id, entity_type: 'client', entity_id: id, action: 'updated' }); revalidatePath('/admin/clients')
}

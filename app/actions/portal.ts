...
type PortalAction = 'communicate' | 'decide'

async function memberContext(action?: PortalAction) {
  ...
  if (!memberships?.length) throw new Error('Client membership required')
  if (action && memberships.every((membership) => membership.portal_role === 'viewer')) throw new Error('Viewer access is read-only')
  return { supabase, user, memberships }
}

export async function createPortalTicket(input: unknown) {
  const data = z.object({ subject: z.string().min(3), description: z.string().min(10), client_id: z.string().uuid(), project_id: z.string().uuid().optional() }).parse(input)
  const { supabase, memberships } = await memberContext('communicate')
  ...
  // Deskripsi disimpan pada ticket utama. Pesan thread pertama dibuat terpisah
  // sehingga kegagalan policy support_messages tidak menggagalkan ticket.
...
export async function decidePortalApproval(approvalId: string, decision: 'approved' | 'changes_requested', note: string) {
  const { supabase, user, memberships } = await memberContext('decide')
  const { data: approval } = await supabase.from('project_approvals').select('id,project_id,business_projects!inner(client_id)').eq('id', approvalId).single()
  if (!approval) throw new Error('Approval not found')
  const project = approval.business_projects as unknown as { client_id: string }
  if (!memberships.some((membership) => membership.client_id === project.client_id)) throw new Error('Forbidden')
  const { error } = await supabase.from('project_approvals').update({ status: decision, decision_note: note, decided_at: new Date().toISOString(), decided_by: user.id }).eq('id', approvalId)
  if (error) throw new Error(error.message)
  revalidatePath('/portal')
  return { ok: true }
}
...
export async function addPortalMessage(input: unknown) {
  ...
  const { supabase, user, memberships } = await memberContext('communicate')
  ...
  if (error) throw new Error(error.message)
  revalidatePath('/portal/support/' + data.ticket_id)
  revalidatePath('/portal')
  return message
}

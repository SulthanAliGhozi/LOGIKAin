...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  const { next, error } = await searchParams; const destination = next === '/admin' ? '/admin' : '/portal'; const message = error === 'staff_required' ? 'Akun belum memiliki akses staff aktif.' : error === 'profile_error' ? 'Profile akun tidak dapat dibaca.' : error === 'membership_required' ? 'Akun belum dihubungkan ke client portal.' : error === 'registration_closed' ? 'Pendaftaran mandiri tidak tersedia. Hubungi tim LOGIKAin untuk mendapatkan akses.' : ''

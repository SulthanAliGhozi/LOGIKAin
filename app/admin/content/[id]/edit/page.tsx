...
export default async function EditContentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ type?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f3f0ea] p-6 text-[#171717] md:p-10">
      <Link href="/admin/content" className="text-xs text-black/50">← Back to content</Link>
      <div className="mt-10">
        <p className="mono text-[10px] text-[#b36f43]">LOGIKAin / CMS / EDIT</p>
        <h1 className="mt-3 text-4xl font-extrabold tracking-[-2px]">Edit {label}</h1>
      </div>
      <div className="mt-10">
        <ContentEditor content={{ ...data, table }} table={table} />
      </div>
    </main>
  )
}

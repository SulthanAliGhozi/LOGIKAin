...
export function BusinessDiagnostic() {
  ...
  if (done) return <div className="border border-ink/15 bg-white/35 p-6 text-ink md:p-8"><p className="mono text-[10px] text-orange">REKOMENDASI AWAL</p><h3 className="mt-4 text-3xl font-extrabold tracking-[-1.5px]">Mulai dari fondasi yang paling berdampak.</h3><p className="mt-4 max-w-xl text-sm leading-7 text-muted">Dari jawaban Anda, kemungkinan besar bisnis membutuhkan pemetaan proses dan satu sistem prioritas sebelum menambah lebih banyak tools.</p><Link href="/start-project" className="mt-7 inline-block bg-ink px-5 py-4 text-xs font-bold text-paper hover:bg-orange hover:text-ink">Bahas hasilnya bersama kami <span className="ml-2">↗</span></Link><button onClick={() => { setAnswers([]); setStep(0) }} className="ml-4 text-xs text-muted underline underline-offset-4">Ulangi</button></div>
